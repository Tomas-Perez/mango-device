...
mango_addr_t* mango_memory_map(mango_addr_t a){
	return ctx.memory + a/sizeof(mango_addr_t);
}
...
mango_exit_t mango_gn_init(char **argv) {
	...
	ctx.memory = (mango_addr_t*) mmap (NULL, ctx.memory_size,
	...
	ctx.event_exit.vaddr=ctx.memory + strtol(argv[2], NULL, 16)/sizeof(mango_addr_t);
	ctx.event_a.vaddr=ctx.memory + strtol(argv[3], NULL, 16)/sizeof(mango_addr_t);
	ctx.event_b.vaddr=ctx.memory + strtol(argv[4], NULL, 16)/sizeof(mango_addr_t);
	ctx.event_r.vaddr=ctx.memory + strtol(argv[5], NULL, 16)/sizeof(mango_addr_t);
